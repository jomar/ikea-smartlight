...
def main():
    ...
    if args.action == 'power':
        if args.value == 'on' or args.value == 'off':
            tradfriActions.tradfri_power_group(hubip, clientid, securityid, args.groupid, args.value)
        else:
            sys.stderr.write('[-] Tradfri: power state can only be on/off\n')
            sys.exit(1)
    elif args.action == 'brightness':
        if 0 <= int(args.value) <= 100:
            tradfriActions.tradfri_dim_group(hubip, clientid, securityid, args.groupid, args.value)
        else:
