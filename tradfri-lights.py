...
def main():
    ...
    if args.action == 'power':
        if args.value == 'on' or args.value == 'off':
            tradfriActions.tradfri_power_light(hubip, clientid, securityid, args.lightbulbid, args.value)
        else:
            sys.stderr.write('[-] Tradfri: power state can only be on/off\n')
            sys.exit(1)
    elif args.action == 'brightness':
        if 1 <= int(args.value) <= 100:
            tradfriActions.tradfri_dim_light(hubip, clientid, securityid, args.lightbulbid, args.value)
        else:
        ...
        if args.value == 'warm' or args.value == 'normal' or args.value == 'cold':
            tradfriActions.tradfri_color_light(hubip, clientid, securityid, args.lightbulbid, args.value)
        else:
